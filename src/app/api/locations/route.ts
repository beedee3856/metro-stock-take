...
export async function PUT(req: Request) {
  try {
    ...
    if (!hasPermission(user.role, "MANAGE_LOCATIONS")) {
      return NextResponse.json({ error: "Forbidden: Only administrators can edit locations" }, { status: 403 });
    }
    ...
    const body = await req.json();
    const { locationCode, locationName, aisle, shelfSection, barcode, description } = body;
    ...
    const existing = await db.select().from(locations).where(eq(locations.id, locationId)).limit(1);
    if (existing.length === 0) {
    ...
    if (barcode) {
      updateFields.barcode = barcode.trim();
    }

    const updated = await db.update(locations).set(updateFields).where(eq(locations.id, locationId)).returning();
    ...
    return NextResponse.json({ success: true, location: updated[0] });
  } catch (error) {
