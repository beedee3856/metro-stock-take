...
export function DashboardView({ onSelectSection, onStartCounting }: DashboardViewProps) {
  ...
  const [hasAssignedRecounts, setHasAssignedRecounts] = useState(false);
  ...
  // Check if stock taker has assigned recounts
  const checkAssignedRecounts = useCallback(async () => {
    if (user?.role === "STOCK_TAKER") {
      try {
        const res = await fetch("/api/recounts?status=ASSIGNED");
        if (res.ok) {
          const json = await res.json();
          setHasAssignedRecounts((json.recounts || []).length > 0);
        }
      } catch (err) {
        console.error("Failed to check recounts", err);
      }
    }
  }, [user?.role]);
  ...
  useEffect(() => {
    fetchStats();
    checkAssignedRecounts();
  }, [fetchStats, checkAssignedRecounts]);
  ...
  const c = data?.charts || {
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-rose-950 p-6 text-white shadow-xl md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {user?.role === "STOCK_TAKER" && (
            <button
              onClick={() => onSelectSection("recounts")}
              disabled={!hasAssignedRecounts}
              className="flex items-center gap-2 rounded-xl bg-amber-600 px-4 py-2.5 text-xs font-semibold text-white shadow-lg shadow-amber-900/30 transition-all hover:bg-amber-700 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <RotateCcw className="h-4 w-4" />
              <span>Recount {hasAssignedRecounts ? "(Pending)" : ""}</span>
            </button>
          )}
      ...
      {/* STOCK TAKER SIMPLIFIED VIEW */}
      {user?.role === "STOCK_TAKER" && (
        <div className="space-y-4">
          <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-50/50 border-2 border-emerald-200 p-6">
            <h2 className="text-lg font-bold text-emerald-900 mb-4">Your Active Assignment</h2>
            {data?.currentStockTake ? (
              <div className="bg-white rounded-xl p-4 border border-emerald-100">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-slate-600 mb-1">Session Code</p>
                    <p className="text-lg font-bold text-slate-900">{data.currentStockTake.number}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-600 mb-1">Location/Session</p>
                    <p className="text-lg font-bold text-slate-900">{data.currentStockTake.name}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-600 mb-1">Status</p>
                    <span className="inline-block rounded-full bg-emerald-100 text-emerald-800 px-3 py-1 text-xs font-bold">
                      {data.currentStockTake.status}
                    </span>
                  </div>
                  <div>
                    <p className="text-xs text-slate-600 mb-1">Count Type</p>
                    <p className="text-sm font-semibold text-slate-900">
                      {data.currentStockTake.isBlindCount ? "Blind Count" : "Verification"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center py-8 text-slate-500">
                <p className="text-sm">No active stock taking session assigned to you</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ADMIN & SUPERVISOR DETAILED VIEW - ONLY SHOWN TO ADMINS */}
      {user?.role !== "STOCK_TAKER" && (
